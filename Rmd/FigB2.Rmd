...
PRPlot <- ggplot(data=normalisedOD %>%
    ...
    axis.title = element_text(size =15, face = "bold", colour = "black", family = "sans"),
...
```{r load_samplesheetb}
#datadir <- "M:data/2022/02-Feb/Laura/Flow_cytometry_data/"
#Will need to make a new sample sheet for each new set of strains/bioreps I do
#straindir <- "M:data/2022/02-Feb/Laura/Flow_cytometry_data/"
# Strain_Order <-  c("KN99ON", "CnLT0004.1ON", "CnLT0004.2ON", "CnLT0004.3ON", "CnLT0007.1ON", "CnLT0007.2ON", "CnLT0007.3ON")
...
mCardinal_Plot <- ggplot(data = dplyr::filter(flow_data_gated_singlets, Strain %in% Strains_A), aes(x = PECy5_A)) +
  ...
  theme(
    ...
    axis.title = element_text(size =15, face = "bold", colour = "black",family = "sans"),
    axis.text = element_text(size = 20,family = "sans"),
    axis.text.x = element_text( hjust =0.5, family = "sans"),
    axis.line = element_line(colour = "black", size = 1, linetype = "solid"),
    panel.background = element_rect(fill = "white"), 
    axis.ticks = element_line(colour = "black", size = 1),
    axis.ticks.length = unit(0.25, "cm"),
...
figure_left_column <- 
    plot_grid(
    PRPlot +
      theme(plot.margin = unit(c(0.1,0,0.5,0.75),"in")),
    mCardinal_Plot,
    theme(plot.margin = unit(c(0.5,0,0,1),"in")),
       ncol = 1,
    rel_heights = c(1,1),
    labels = c("B","C")
  )
...
```{r save}
ggsave("../Output/figure_left_column .jpeg", 
figure_left_column ,width = 7,height=10)
```
